...
Сделай отчет ВИЗУАЛЬНО КРАСИВЫМ!`
  ...
  /**
   * Генерирует HTML дашборда для региона
   * Используется для дашборда БЕЗ привязки к конкретным компаниям
   */
  async generateDashboard(marketData: any): Promise<string> {
    return this.execute(async () => {
      this.log('Generating dashboard HTML from market data');

      // Вызываем AI агента для создания HTML дашборда
      const response = await this.invokeAgent(
        `Создай ВИЗУАЛЬНО БОГАТЫЙ HTML дашборд для IT-рынка Татарстана.

Используй следующие рыночные данные:
${JSON.stringify(marketData, null, 2)}

Используй ВСЕ доступные компоненты (tools):
1. get_card - для ключевых метрик (работодатели, вакансии, средняя зарплата, тренд рынка)
2. get_section - для заголовка "🏆 Топ работодатели по вакансиям"
3. get_list - для списка работодателей и трендов
4. get_chart - для топ технологий по спросу (bar chart)

Структура дашборда:
1. Ключевые метрики (4 карточки в сетке)
2. Топ работодатели (секция + список)
3. Топ технологии (график)
4. Рыночные тренды (список)

В конце оберни ВСЁ в: <div class="content-wrap">...</div>
...
НЕ генерируй HTML руками - ТОЛЬКО через tools!`
  ...
  /**
   * Парсит ответ агента для дашборда
   */
  private parseDashboardResponse(response: any): string {
    ...
    return `<div class="content-wrap">
      <h2>🎯 IT-рынок Татарстана</h2>
      <p>HTML дашборд сгенерирован AI агентом через tools</p>
      <p><em>TODO: парсинг результатов tools из LangChain response</em></p>
    </div>`;
  }
